...
function loadAppData() {
  try {
    ...
    // Migrate old single-tournament data if present
    const oldData = localStorage.getItem('apex_bracket_state');
    if (oldData && !Object.keys(appData.tournaments).length) {
      try {
        const old = JSON.parse(oldData);
        if (old && old.players) {
          const id = `t_migrated_${Date.now()}`;
          appData.tournaments[id] = {
            id, name: 'Migrated Tournament', createdAt: Date.now(),
            bracketType: old.bracketType || 'single',
            bracketSizeConfig: 'auto',
            bracketSize: old.bracketSize || 8,
            status: old.status || 'setup',
            players: old.players || [],
            bracket: old.bracket || null,
            registrationTimer: old.registrationTimer || { duration: 60, remaining: 60, isActive: false }
          };
...
function routeApp() {
  ...
  if (params.has('view') && params.get('view') === 'live') {
    ...
    state = (tid && appData.tournaments[tid]) ? appData.tournaments[tid] : null;
    if (!state && appData.currentId) state = appData.tournaments[appData.currentId] || null;
...
function openQRRegistration() {
  ...
  if (qrImg) qrImg.src = `https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=${encodeURIComponent(regUrl)}`;
  ...
  form.addEventListener('submit', (e) => {
    ...
    try {
      ...
      if (saved) {
        ...
        const tid = new URLSearchParams(window.location.search).get('tid') || fresh.currentId;
        if (tid && fresh.tournaments[tid]) state = fresh.tournaments[tid];
      }
    } catch(err) {}

    if (!state) { showRegMsg('Registration is not available.', 'error'); return; }
    if (state.status !== 'registration') { showRegMsg('Registration is closed.', 'error'); return; }
    if (!state.registrationTimer?.isActive && state.registrationTimer?.duration > 0) {
      showRegMsg('Registration time has expired.', 'error'); return;
    }
    ...
    // Check capacity only for fixed sizes
    if (state.bracketSizeConfig !== 'auto' && state.players.length >= parseInt(state.bracketSizeConfig || 64)) {
      showRegMsg(`Registration full (${state.bracketSizeConfig} players max).`, 'error'); return;
    }

    state.players.push({ name, companyId, status: 'active' });
    // Save back to app data
    try {
      const saved = localStorage.getItem(STORAGE_KEY);
      const fresh = saved ? JSON.parse(saved) : appData;
      const tid = new URLSearchParams(window.location.search).get('tid') || fresh.currentId;
      if (tid) fresh.tournaments[tid] = state;
